// LICENSE LGPL-2.1-or-later
{
    ...
    let Typer
    let GenJs
    /*declare encodeURIComponent*/
    /*declare decodeURIComponent*/
    /*declare typeof*/
    /*declare __js__*/
    /*declare eval*/
    /*declare Int*/
    ...
    TestParser.test = () => {
        ...
        TestParser.shouldAllEqual(new Map([
            ['', '<!--null-->'],
            ['  ', '<!--null-->'],
            ['	', '<!--null-->'],
            ['   	', '<!--null-->'],
        ...
        TestParser.shouldAllEqual(new Map([
            ['1 2 3 trace("Hello!", "World!") + 5 * 6 / 3', 'TBlock([\r\n				TInt(1),\r\n				TInt(2),\r\n				TInt(3),\r\n				TBinop(\r\n				TCall(TIdent(trace),[TString(Hello!),TString(World!)]),\r\n				+,\r\n				TBinop(TInt(5),*,TBinop(TInt(6),/,TInt(3))))])'],
            ['enum Test { Demo } hello World', 'TBlock([TEnum(Type(Test),[TIdent(Demo)]),\r\n				TIdent(hello),\r\n				TIdent(World)])']
        ]));
        TestParser.shouldAllEqual(new Map([
        ...
        TestParser.shouldAllEqualWithoutTrim(new Map([
            ['"\\( v )"', 'TParenthesis(TBinop(TString(),+,TBinop(TParenthesis(TIdent(v)),+,TString())))'],
        ...
        TestParser.shouldAllEqual(new Map([
            ['var a = new Array<A>()', 'TVar(a,null,TNew([],ParametricType(Array,[Type(A)]),[],[],[]),false)'],
            ...
            ['a = EnumTest<A,B>.EnumField', 'TBinop(TIdent(a),=,TDot(NodeTypeValue(ParametricType(EnumTest,[Type(A),Type(B)])),EnumField))'],
        ...
        TestParser.shouldAllEqual(new Map([
            ['var x:[Array<T>]', 'TVar(x,ParametricType(Array,[ParametricType(Array,[Type(T)])]),null,false)'],
            ['var x:[Map<K,V> : Array<T>]', 'TVar(x,ParametricType(Map,[ParametricType(Map,[Type(K),Type(V)]),ParametricType(Array,[Type(T)])]),null,false)'],
            ['var x:{:}, y:[], z:[:], w:()=>{:}', 'TVars([TVar(x,Object([],[]),null,false),TVar(y,ParametricType(Array,[Object([],[])]),null,false),TVar(z,ParametricType(Map,[Object([],[]),Object([],[])]),null,false),TVar(w,Function([],Object([],[])),null,false)])'],
        ...
        TestParser.shouldAllEqual(new Map([
            ...
            ['enum A { A(v:Int) B C(v:[K:V], a:Array<T>) }', 'TEnum(Type(A),[TCall(TIdent(A),[v:TIdent(Int)]),TIdent(B),TCall(TIdent(C),[v:TMap([TIdent(K)],[TIdent(V)]),a:NodeTypeValue(ParametricType(Array,[Type(T)]))])])']
        ]));
        TestParser.shouldAllEqual(new Map([
            ['class A {} var a = new A { } ()', 'TBlock([TClass(Type(A),null,[],[],false),TVar(a,null,TNew([],Type(A),[],[],[]),false)])'],
            ['class A { var field: String } var a = new A { field: "Value" } ()', 'TBlock([TClass(Type(A),null,[],[TVar(field,Type(String),null,false)],false),TVar(a,null,TNew([],Type(A),[],[field],[TString(Value)]),false)])'],
            ['class A { var field: String var otherfield: Int } var a = new A { field: "Value", otherfield: 25 } ()', 'TBlock([TClass(Type(A),null,[],[TVar(field,Type(String),null,false),TVar(otherfield,Type(Int),null,false)],false),TVar(a,null,TNew([],Type(A),[],[field,otherfield],[TString(Value),TInt(25)]),false)])']
        ]));
        TestParser.shouldAllEqual(new Map([
                ...
                case 10:
                    {
                        const el = $switch$[2]; {
                            ...
                            GenJs.pushTab(); {
                                ...
                                for (const ns of intermediate)
                                    if (ns == '') {} else if (ns.indexOf('.') == -1) {
                                ...
                                for (const e$109 of el) {
                                    ...
                                    switch ((($switch$ == undefined) || (["number", "string"].indexOf(typeof($switch$)) != -1)) ? $switch$ : $switch$[1]) {
                                        ...
                                        default:
                                            {
                                                output.push('\t/*!22*/' + GenJs.stringifyBlockExpression(e$109));
                                            }
                                    }
                                };
                            };
                                                ...
                                                const t = $switch$[2]; {
                                                    {
                                                        ...
                                                        switch ((($switch$ == undefined) || (["number", "string"].indexOf(typeof($switch$)) != -1)) ? $switch$ : $switch$[1]) {
                                                            ...
                                                            default:
                                                                {
                                                                    throw 'Expected NodeType.Type' + t;
                                                                }
                                                        }
                                                    };
                                                }
                ...
                case 28:
                    {
                        ...
                        const values = $switch$[3]; {
                            ...
                            return 'new Map([' + ((() => { const return$$ = []; const $switch$ = keys.length; let i = -1; while ((i + 1) < $switch$) { i++;
                                    return$$.push('[' + GenJs.unwrapBlockValue(keys[i]) + ', ' + GenJs.unwrapBlockValue(values[i]) + ']'); } return return$$; })()).join(', ') + '])';
                        }
                        break;
                    }
                case 12:
                ...
                case 32:
                    {
                        const names = $switch$[2];
                        const el = $switch$[3]; {
                            return '{' + ((() => { const return$$ = []; const $switch$ = el.length; let i = -1; while ((i + 1) < $switch$) { i++;
                                    return$$.push((GenJs.rename(names[i]) + ':' + GenJs.unwrapBlockValue(el[i]))); } return return$$; })()).join(', ') + '}';
                        }
                        break;
                    }
                case 40:
    ...
    class Main {
        ...
        init() {
            ...
            let name$164 = Main.question(('Enter package name (' + (currentDir) + '): '));
            if (name$164 == '') {
                name$164 = currentDir;
            };
        ...
        repl() {
            ...
            let lines$168 = [];
            while (true) {
                let line$169 = Main.question('> ').trim();
                if (line$169.length == 0) {
                    continue;
                };
                lines$168.push(line$169);
                lines$168 = [line$169];
                try {
                    let code = '"use strict"\r\n';
                    try {
                        const content = lines$168.join('\n');
                        const tokens$170 = Lexer.tokenize(Buffer.from(content), 'REPL');
                        const parser = new Parser(tokens$170);
                        if (parser.node == null) {
                            continue;
                        };
                        let collected = Node.TBlock([js, nodejs, Node.TModule([], []), parser.node]);
                        Typer.fillScopes(collected);
                        code += GenJs.stringifyProject(collected)
                    } catch (e) {
                        lines$168.pop();
        ...
        processFiles(targets) {
            let res = [];
            for (const target$171 of targets) {
                try {
                    res.push(this.processFile(target$171))
                } catch (e) {
        ...
        loadPackage(filename$172) {
            let pack$173 = JSON.parse(fs.readFileSync(path.resolve(filename$172)).toString('utf8'));
            const incorrect = (what) => {
                process.stdout.write(('Incorrect package file! Field `' + (what) + '` does not exists\n'));
                process.exit(1);
            };
            if (pack$173.target == null) {
                incorrect('target');
            };
            if (pack$173.main == null) {
                incorrect('main');
            };
            if (pack$173.name == null) {
                incorrect('name');
            };
            console.log(path.resolve('library/js/js.hexa'));
            console.log(path.resolve('library/nodejs/nodejs.hexa'));
            if (pack$173.files == null) {
                pack$173.files = [];
            };
            pack$173.files.unshift(path.resolve('library/js/js.hexa'));
            pack$173.files.unshift(path.resolve('library/nodejs/nodejs.hexa'));
            return pack$173;
        }
        constructor() {

            {
                ...
                if (currentParsedFile.ext == '.json') {
                    input$174 = this.loadPackage(currentFile);
                    console.log(('[Building ' + (input$174.name) + ']'));
                    main = path.resolve(currentFile + '/../') + path.sep + (input$174.main);
                } else if (fs.lstatSync(currentFile).isDirectory()) {
                    input$174 = this.loadPackage(currentFile + path.sep + 'hexa.json');
                    packageFolder = currentFile;
                    main = packageFolder + path.sep + (input$174.main);
                } else if (currentParsedFile.ext == '.hexa') {
                    eval('"use strict"\r\n' + GenJs.stringifyProject(this.processFile(currentFile)));
                    return;
                };
                let codes = [];
                if (input$174.files != null) {
                    let files = input$174.files.map((file) => {
                ...
                let parsedMain = collected; {
                    const $switch$ = input$174.target.generator;
                    switch ((($switch$ == undefined) || (["number", "string"].indexOf(typeof($switch$)) != -1)) ? $switch$ : $switch$[1]) {
                        case Generator.JavaScript:
                            {
                                let outs = GenJs.stringifyProject(collected);
                                let target$175 = packageFolder + path.sep + path.parse(input$174.main).name + '.js';
                                if (input$174.output != null) {
                                    target$175 = packageFolder + path.sep + path.parse(input$174.output).name + '.js';
                                };
                                const license = input$174.license != (null) ? (('// LICENSE ' + (input$174.license) + '\n')) : ('');
                                fs.writeFileSync(target$175, license + outs);
                                if (input$174.target.instance != null && input$174.target.instance.generatePackageJson) {
                                    fs.writeFileSync(packageFolder + path.sep + 'package.json', GenJs.generatePackageJson(input$174));
                                };
                            }
                            break;
                        case Generator.C:
